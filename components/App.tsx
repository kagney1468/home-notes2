...
const App: React.FC = () => {
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setSecondReport(null);
      setStatus(AppState.REPORT);
      if (currentUser) saveReport(data, currentUser.id, currentUser.email!);
    } catch (err: unknown) {
      setError((err as Error).message || 'An unexpected error occurred.');
      setStatus(AppState.ERROR);
    }
  ...
  const whatsappUrl = `https://wa.me/447724601320?text=Hi Home Sales Ready, I have a question about my Home Notes property report.`;
    ...
    <div className="min-h-screen flex flex-col text-slate-900">
      ...
      <header className="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 h-16 flex items-center justify-between">
          ...
          <nav className="flex items-center gap-3 md:gap-4">
            ...
            <button onClick={() => setShowProfessionalAgent(true)}
      ...
      <main className="flex-grow">
        ...
        {(status === AppState.REPORT || status === AppState.COMPARING) && report && (
          <ReportDashboard report={report} secondReport={secondReport}
            onNewSearch={handleReset} onCompare={handleCompare} isComparing={isComparing} />
        )}
        ...
        <div className="fixed inset-0 z-[80] flex items-center justify-center p-4">
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl max-w-lg w-full p-8">
            ...
            <div className="space-y-4 text-sm text-slate-600 leading-relaxed">
              <p><strong className="text-slate-900">Home Notes uses AI language models</strong> to generate property area reports. Data may not reflect current, accurate, or real-world conditions.</p>
              <p>Before making any property decision always verify from official sources (Ofsted, Environment Agency, Ofcom, ONS) and consult qualified professionals.</p>
      ...
      {showMyReports && currentUser && (
        <MyReports userId={currentUser.id} userEmail={currentUser.email!} onClose={() => setShowMyReports(false)}
          onLoadReport={handleLoadSavedReport} />
      )}

      {showProfessionalAgent && <ProfessionalInsightsAgent onClose={() => setShowProfessionalAgent(false)} />}
      ...
      <a href={whatsappUrl} target="_blank" rel="noopener noreferrer"
        ...
        <span className="max-w-0 overflow-hidden whitespace-nowrap group-hover:max-w-xs transition-all duration-500 text-sm font-bold">WhatsApp Us</span>
      </a>
